...
Since AiiDAlab is a jupyter notebook environment it is recommended to run AiiDAlab through a container engine
...
`AiiDAlab Launch <https://aiidalab.readthedocs.io/en/latest/usage/access/launch.html>`_ which provides a
...
configurations. To create a new profile and start the AiiDAlab interface run,
...
- ``AIIDA_USER_FIRST_NAME``: User's first name
